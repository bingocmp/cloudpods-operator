...
func (acnt *SCloudaccount) importSubAccount(ctx context.Context, userCred mcclient.TokenCredential, subAccount cloudprovider.SSubAccount) (*SCloudprovider, bool, error) {
	...
	if providerCount == 1 {
		...
		provider.SetMetadata(ctx, "AccountId", subAccount.Id, userCred)
		db.Update(provider, func() error {
	...
	// 关联默认项目
	domainId := acnt.DomainId
	projectId := acnt.ProjectId
	...
	newCloudprovider, err := func() (*SCloudprovider, error) {
		...
		newCloudprovider.SetMetadata(ctx, "AccountId", subAccount.Id, userCred)
...
func (acnt *SCloudaccount) PerformCreateSubscription(ctx context.Context, userCred mcclient.TokenCredential, query jsonutils.JSONObject, input api.SubscriptonCreateInput) (jsonutils.JSONObject, error) {
	...
	} else if acnt.Provider == api.CLOUD_PROVIDER_BINGO_CLOUD {
		if len(input.SubAccountId) == 0 {
			return nil, httperrors.NewMissingParameterError("sub_account_id")
		}
		if len(input.DefaultProject) == 0 {
			input.DefaultProject = identity.DEFAULT_PROJECT
		}
		if len(input.SubAccountName) == 0 {
			input.SubAccountName = input.SubAccountId
		}
	} else {
